# SVGのガイドライン
- [SVGのガイドライン](#svgのガイドライン)
  - [SVGの作成時に注意すること](#svgの作成時に注意すること)
    - [サイズ削減のためにすること](#サイズ削減のためにすること)
    - [正常に表示するためにすること](#正常に表示するためにすること)
    - [書き出し設定で注意すること](#書き出し設定で注意すること)
  - [コードの最適化をする](#コードの最適化をする)
    - [不要なコードを削除する](#不要なコードを削除する)
    - [必須の属性を確認する](#必須の属性を確認する)
    - [gulp-imageminの設定例](#gulp-imageminの設定例)
  - [imgタグでSVGを表示する](#imgタグでsvgを表示する)
    - [フルードイメージ](#フルードイメージ)
    - [バグフィックス（IEとAndroid）](#バグフィックスieとandroid)
    - [フォールバック（IE8）](#フォールバックie8)
      - [objectタグでフォールバックする](#objectタグでフォールバックする)
      - [JavaScriptでフォールバックする](#javascriptでフォールバックする)
  - [background-imageでSVGを表示する](#background-imageでsvgを表示する)
  - [svgタグでSVGを表示する](#svgタグでsvgを表示する)
    - [svgタグのアクセシビリティ](#svgタグのアクセシビリティ)
  - [SVGにリンクを設定する](#svgにリンクを設定する)
  - [参考リンク](#参考リンク)
