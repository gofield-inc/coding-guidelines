...
- [HTMLのガイドライン](#htmlのガイドライン)
  ...
  - [フォーマッティング](#フォーマッティング)
    ...
    - [空行と改行とインデントを入れて整形する](#空行と改行とインデントを入れて整形する)
  ...
  - [HTMLタグ](#htmlタグ)
    ...
    - [画像の切り替えはpictureタグやsrcset属性を使う](#画像の切り替えはpictureタグやsrcset属性を使う)
  ...
  - [headタグ内](#headタグ内)
    ...
    - [ファビコンはheadタグ内で読み込まない](#ファビコンはheadタグ内で読み込まない)
    - [必須のメタタグ](#必須のメタタグ)
      - [title](#title)
...
トップページはサイトのタイトルやロゴ、記事ページは記事タイトル、それ以外のページはページのタイトルをh1にします。
...
### 外部リンク、PDFリンクにはアイコン付きで`target="_blank"`を指定する（アクセシビリティ検証がない案件のみ）
...
CMSではブログネームを取得するなど、一括して変更できる仕様にします。
